...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ZMY
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ZMY
    // Mac: ~/Library/Application Support/ZMY
    // Unix: ~/.ZMY
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ZMY";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "ZuMY.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "ZuMYd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong ZuMY will not work properly.");
